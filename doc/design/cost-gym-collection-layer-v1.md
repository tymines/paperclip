...
Timestamp grammar (top-level and per-observation `observedAt`, PR #27 revision 4 — AUTONOMOUS GAP-FILL E): `YYYY-MM-DDTHH:mm:ss[.fraction](Z | ±HH:mm)`, validated by shape (regex), calendar (month/day ranges including leap years, so `2026-02-30` rejects even though V8 `Date.parse` normalizes it), and `Date.parse` range checks for time/offset. Parseable-but-non-ISO forms (`July 28, 2026`, `2026/07/28`, RFC-822 dates), date-only forms, missing seconds, space separators, and colon-less offsets all reject the envelope. The prior `Date.parse`-only check accepted those non-ISO forms and did not implement the claimed ISO boundary.
