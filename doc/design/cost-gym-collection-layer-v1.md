...
`fleet-observation/v1` observations are validated completely per discriminated `payloadKind` (`cost.usage.session`, `cost.usage.model`, `cost.speed.api_call`) before ingest or dedupe: required ids/strings, timestamps, billing enum fields, nullable fields, finite numeric types, and nonnegative counts/durations/tokens, plus the observation wrapper (`observedAt`, `checkpoint`, `freshness`) and the box/source invariant (observation source must match the envelope source box/collector AND profile identity exactly under nullable semantics — AUTONOMOUS GAP-FILL B, PR #27 revision 3). The envelope's own top-level trusted metadata is validated completely before the typed envelope is returned or the store is touched: `observedAt` must be a strict ISO 8601 / RFC 3339 date-time string (see below), `checkpoint` must be `{ sequence: nonnegative integer, cursor: string | null }`, and `freshness` must be `{ errors: string[] }`. Any missing, string, `NaN`/`Infinity`, negative, malformed, or wrong-kind field rejects the whole envelope with `FleetObservationEnvelopeError` identifying the observation, kind, and field. Nothing invalid enters the dedupe store and there is no partial normalization.

Timestamp grammar (top-level and per-observation `observedAt`, PR #27 revision 4 — AUTONOMOUS GAP-FILL E; range checks corrected in round 5, calendar check corrected in round 6): `YYYY-MM-DDTHH:mm:ss[.fraction](Z | ±HH:mm)`, validated by shape (regex), calendar (month/day ranges including leap years, so `2026-02-30` rejects even though V8 `Date.parse` normalizes it), and explicit numeric range checks for hour (00-23), minute (00-59), second (00-59), and numeric offset (±00:00 through ±23:59). The calendar check is pure proleptic-Gregorian arithmetic over the captured four-digit year (0000-9999, RFC 3339 section 1): leap iff `year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)` (RFC 3339 Appendix C, so year 0000 is leap and `0000-02-29` accepts, while non-leap centuries such as 0100/1900 reject `02-29` and leap centuries 0400/2000 accept it). No `Date`/`Date.UTC` is used for the calendar boundary: JavaScript remaps years 0-99 to 1900-1999, so the round-5 `Date.UTC`-based days-in-month check wrongly rejected the valid leap day `0000-02-29T00:00:00Z` (Chronos PR #27 round-6 P1). Ranges are NOT delegated to `Date.parse`: V8 accepts `24:00:00` and normalizes it into the next day, so the round-4 implementation wrongly accepted `2026-07-28T24:00:00Z` (Chronos PR #27 round-5 P1). Leap seconds (`:60`) are rejected by this profile — narrower than RFC 3339, which permits them at announced leaps; this matches the prior runtime behavior. Parseable-but-non-ISO forms (`July 28, 2026`, `2026/07/28`, RFC-822 dates), date-only forms, missing seconds, space separators, and colon-less offsets all reject the envelope. The prior `Date.parse`-only check accepted those non-ISO forms and did not implement the claimed ISO boundary.
