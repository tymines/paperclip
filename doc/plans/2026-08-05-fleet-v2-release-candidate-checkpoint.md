...
- Release candidate: the immutable commit containing this checkpoint; resolve it with `git rev-parse HEAD` in this worktree and use the SHA recorded in the handoff.
...
- `pnpm --filter @paperclipai/ui exec vitest run src/pages/Agents.test.tsx` — pass: 1 file, 5 tests.
...
- `git diff --check` — pass.
- Blob-equivalence audit — pass: the seven Fleet-only paths match `79692ca57d8ce08a47e3ed1fa32b183ebd13dd0d`; the three overlapping runtime/test paths plus the security checkpoint match the four-path delta from `c9d43ac6c2f5b41333f336ea088cb69f0c812f8d`.
...
- Safe UI check — pass through the focused jsdom Agents rendering tests, including the exact 16 rows, host groups, registered/unregistered behavior, company ID request, fallback behavior, and org view.
...
2. Fetch and verify that `RC_SHA` is a commit, that its only parent is `826238c0f5cb211c947001f0847f75d511edbb4f`, and that neither blocked Book commit is its ancestor.
...
7. Run authenticated, read-only activation checks:
   ...
   - Agents UI: selected company remains unchanged, the Fleet summary reports 16 positions, Windows/Box 1/Box 2 groups match this checkpoint, and no pause/resume control is used during the check.
