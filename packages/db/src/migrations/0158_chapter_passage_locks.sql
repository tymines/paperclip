...
-- FAIL-CLOSED backfill (Spec v1 §7): pre-existing vault `human_locked: true`
-- frontmatter is imported into this column at runtime by
-- server/src/services/book-locks.ts#backfillChapterLocksFromVault (SQL cannot
-- read the vault filesystem). The import runs once per book before any lock
-- enforcement decision and syncs strictly upward (false→true). Additionally,
-- every enforcement read honors vault `human_locked` LIVE, and the shared
-- prose sink (persistChapterProse) never writes a chapter whose vault
-- frontmatter says human_locked — so a chapter that was human-locked before
-- this migration can never be silently unlocked or overwritten by the AI.
