...
const EXISTING = { id: "book-1", companyId: "company-1", slug: "stable-slug", title: "Old Title", metadata: { description: "premise" }, createdAt: new Date(), updatedAt: new Date() };
...
describe("Book Studio rename", () => {
  ...
  it("renames only the display title, preserves slug/content metadata, and activity-logs the mutation", async () => {
    ...
    const response = await request(app).patch("/api/companies/company-1/book-studio/books/book-1").send({ title: "  New Title  " }).expect(200);
  ...
  it("rejects empty titles without updating", async () => {
    ...
    await request(app).patch("/api/companies/company-1/book-studio/books/book-1").send({ title: "   " }).expect(400);
  ...
  it("returns not found for a book outside the company and performs no mutation", async () => {
    ...
    await request(app).patch("/api/companies/company-1/book-studio/books/book-1").send({ title: "Nope" }).expect(404);
    expect(db.update).not.toHaveBeenCalled();
  });
});
