...
export function loadConfig(): Config {
  ...
  // BUILD-GATE: fail loudly when postgres mode is configured but no connection string is available.
  // NEVER silently fall back to embedded PostgreSQL — this is what makes the :5432 pin bulletproof.
  if (fileDatabaseMode === "postgres") {
    const effectiveUrl = process.env.DATABASE_URL ?? fileDbUrl;
    if (!effectiveUrl) {
      throw new Error(
        "BUILD-GATE: Database mode is configured as \"postgres\" but no connection string was found. " +
        "This server will not silently fall back to embedded PostgreSQL. " +
        "Set DATABASE_URL environment variable or configure database.connectionString in config.json to " +
        "point to your external PostgreSQL instance (default: postgres://paperclip:***@127.0.0.1:5432/paperclip).",
      );
    }
  }
