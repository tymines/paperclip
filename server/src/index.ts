...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.databaseUrl) {
    ...
    if (config.databaseMode === "postgres") {
      // BUILD-GATE: fail loudly — never silently fall back to embedded PostgreSQL.
      // This is the runtime safety net should the config.ts guard be bypassed.
      throw new Error(
        "BUILD-GATE: Database mode is configured as \"postgres\" but no connection string was set. " +
        "The server will NOT silently fall back to embedded PostgreSQL. " +
        "Set DATABASE_URL environment variable or configure database.connectionString in config.json.",
      );
    }
  ...
  setupRoomWebSocketServer(server, db as any, {
    deploymentMode: config.deploymentMode,
    resolveSessionFromHeaders,
  });
