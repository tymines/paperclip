// Story Bible codex routes (Spec v1 §4.1) — migration 0159-gated.
...
export function bookStudioCodexRoutes(db: Db) {
  ...
  const codex503 = () => serviceUnavailable(PENDING_0159);
  ...
  router.get(`${BASE}/codex/:entityType`, async (req, res, next) => {
    try {
      ...
      try {
        ...
        res.json({ available: false, pendingMigration: "0159", entityType, entities: [] });
      }
    } catch (err) { next(err); }
  });
  ...
  router.get(`${BASE}/codex-relationships`, async (req, res, next) => {
    try {
      ...
      try {
        ...
        if (!isMissingCodexTable(err)) throw err;
        res.json({ available: false, pendingMigration: "0159", relationships: [] });
      }
    } catch (err) { next(err); }
  });
