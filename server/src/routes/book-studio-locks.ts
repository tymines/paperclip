...
const PENDING_0158 =
  "passage_locks table pending migration 0158 — passage locks unavailable until it is applied.";
  ...
  router.get(`${BASE}/chapters/:chapterNumber/locks`, async (req, res, next) => {
    try {
      ...
      res.json({
        ...
        passageLocks: locks,
        available,
        ...(available ? {} : { pendingMigration: "0158" }),
      });
    } catch (err) { next(err); }
  });
  ...
  router.post(`${BASE}/chapters/:chapterNumber/passage-locks`, async (req, res, next) => {
    try {
      ...
      try {
        ...
        throw serviceUnavailable(PENDING_0158);
      }
    } catch (err) { next(err); }
  });
