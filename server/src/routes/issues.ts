...
export function issueRoutes(
  ...
  router.post("/issues/:id/children", applyCreateIssueStatusDefault, validate(createChildIssueSchema), async (req, res) => {
    ...
    const actor = getActorInfo(req);
    const runOwnership = req.actor.type === "agent" && actor.agentId === parent.assigneeAgentId
      ? { agentId: actor.agentId!, runId: actor.runId! }
      : undefined;
    const executionPolicy = applyActorMonitorScheduledBy(
      normalizeIssueExecutionPolicy(req.body.executionPolicy),
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    if (commentBody) {
      ...
      comment = await svc.addComment(id, commentBody, {
        ...
        runId: actor.runId,
      }, { enforceRunOwnership: req.actor.type === "agent" && actor.agentId === issue.assigneeAgentId });
    ...
    void (async () => {
      ...
      if (commentBody && comment) {
        ...
        // Fallback: no assignee but user-commented → route to Zeus (triage orchestrator)
        if (!assigneeId && !isClosed && !actorIsAgent && !assigneeChanged) {
          try {
            const triageResult = await agentsSvc.resolveByReference(issue.companyId, "Zeus");
            const triageAgent = triageResult.agent;
            if (triageAgent && !triageResult.ambiguous) {
              addWakeup(triageAgent.id, {
                source: "automation",
                triggerDetail: "system",
                reason: "issue_commented",
                payload: {
                ...
                contextSnapshot: {
                  ...
                  commentId: comment.id,
                  wakeCommentId: comment.id,
                  source: "issue.comment.unassigned",
                  wakeReason: "issue_commented_unassigned",
                },
              });
            }
          } catch (err) {
            logger.warn({ err, issueId: id }, "failed to resolve fallback triage agent (Zeus)");
          }
        }
  ...
  router.post("/issues/:id/comments", validate(addIssueCommentSchema), async (req, res) => {
    ...
    void (async () => {
      ...
      // Fallback: no assignee but user-commented → route to Zeus (triage orchestrator)
      if (!assigneeId && !isClosed && !actorIsAgent) {
        try {
          const triageResult = await agentsSvc.resolveByReference(currentIssue.companyId, "Zeus");
          const triageAgent = triageResult.agent;
          if (triageAgent && !triageResult.ambiguous && !wakeups.has(triageAgent.id)) {
            wakeups.set(triageAgent.id, {
              source: "automation",
              triggerDetail: "system",
              reason: "issue_commented",
              payload: {
  ...
  router.delete("/attachments/:attachmentId", async (req, res) => {
    ...
    const removed = await svc.removeAttachment(attachmentId, { runOwnership });
    if (!removed) {
      res.status(404).json({ error: "Attachment not found" });
      return;
    }
