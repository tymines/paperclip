...
import { companies, companySkills, skillUsageEvents, type SkillUsageEvent } from "@paperclipai/db";
...
export function companySkillService(db: Db) {
  ...
  // Ensure the skill_usage_events table exists, even if the migration is stuck.
  // This runs once per service instantiation and is idempotent.
  (async () => {
    try {
      const rawSql = (db as any).$client;
      if (rawSql && typeof rawSql.unsafe === "function") {
        ...
        `);
        await rawSql.unsafe(`CREATE INDEX IF NOT EXISTS sue_skill_id_idx ON skill_usage_events (company_id, skill_id)`);
        await rawSql.unsafe(`CREATE INDEX IF NOT EXISTS sue_company_idx ON skill_usage_events (company_id)`);
        await rawSql.unsafe(`CREATE INDEX IF NOT EXISTS sue_created_at_idx ON skill_usage_events (company_id, skill_id, created_at)`);
      }
    } catch {
      // Table creation is best-effort for dev robustness.
    }
  })();
  ...
  async function pruneMissingLocalPathSkills(companyId: string) {
    const rows = await db
      ...
      .where(
        and(
          ...
          sql`key LIKE 'paperclipai/paperclip/%'`
        )
      );
    ...
    // BUILD-GATE: NEVER prune skills with sourceKind === "paperclip_bundled".
    // Bundled skills are synced from the Paperclip repo and managed by
    // ensureBundledSkills(). Auto-deleting them is wrong in any case:
    //   - On a cross-OS migration (Mac→Windows) ALL local paths will be
    //     unresolvable, causing a full wipe of the bundled catalog.
    //   - These skills are the Paperclip repo's canonical set — they should
    //     be re-synced, never pruned.
    const candidates = skills.filter((s) => {
      const meta = s.metadata as Record<string, unknown> | null;
      if (meta?.sourceKind === "paperclip_bundled") {
        console.warn(
          `[BUILD-GATE] Skipping prune of paperclip_bundled skill: key=${s.key} id=${s.id}`
        );
        return false;
      }
      return true;
    ...
    const missingIds = new Set(await findMissingLocalSkillIds(candidates));
    if (missingIds.size === 0) return;

    for (const skill of candidates) {
      if (!missingIds.has(skill.id)) continue;
      // BUILD-GATE: double-check key prefix before deletion (belt + suspenders).
      if (!skill.key.startsWith("paperclipai/paperclip/")) {
        console.warn(
          `[BUILD-GATE] Skipping prune of non-Paperclip key: key=${skill.key} id=${skill.id}`
        );
        continue;
      }
      await db
  ...
  /**
 * Compute real 30-day usage stats from the skill_usage_events table.
 * Returns empty stats when no events exist — never backfills or synthesizes.
 * Gracefully handles the case where the table doesn't exist yet (dev setup).
 */
async function computeUsage30dForSkill(companyId: string, skillId: string): Promise<CompanySkillUsageStats> {
  const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  let countResult: Array<{ count: number }> | null = null;
  try {
    countResult = await db
      ...
      .where(
        and(
          ...
          gte(skillUsageEvents.createdAt, thirtyDaysAgo),
        ),
      );
  } catch {
  ...
  const invocationCount = Number(countResult?.[0]?.count ?? 0);
  if (invocationCount === 0) return emptyUsageStats();

  // Count successes within 30 days for success rate
  let successCount = 0;
  try {
    ...
    successCount = Number(successResult?.[0]?.count ?? 0);
  } catch {
    successCount = 0;
  }
  ...
  return {
    ...
    successRate: invocationCount > 0 ? successCount / invocationCount : null,
    avgLatencyMs: null,
    totalCostCents: 0,
  };
}
...
async function list(companyId: string): Promise<CompanySkillListItem[]> {
    ...
    return Promise.all(rows.map(async (skill) => {
      ...
      const usage30d = await computeUsage30dForSkill(companyId, skill.id);
      return toCompanySkillListItem(skill, attachedAgentCount, totalAgentCount, usage30d);
    }));
  }
  ...
  return {
    ...
    // ── Skill usage event logging ──────────────────────────────────────

    async recordUsageEvent(
      companyId: string,
      skillId: string,
      input: CompanySkillRecordUsageEventRequest,
    ): Promise<SkillUsageEvent | null> {
      try {
    ...
    async listUsageEvents(
      ...
      limit = 50,
      offset = 0,
    ): Promise<CompanySkillUsageEventListResponse> {
