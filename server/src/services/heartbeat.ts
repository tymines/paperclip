...
function normalizeBilledCostCents(
  ...
  if (inputTokens > 0 || outputTokens > 0) {
    const priced = computeCostCents(model, inputTokens, cachedInputTokens, outputTokens);
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  async function updateRuntimeState(
    ...
    const additionalCostCents = normalizeBilledCostCents(result.costUsd, billingType, result.model ?? "unknown", inputTokens, cachedInputTokens, outputTokens);
