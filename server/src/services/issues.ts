...
const ALL_ISSUE_STATUSES = ["backlog", "todo", "in_progress", "in_review", "needs_approval", "blocked", "done", "changes_requested", "cancelled"];
...
function runningIssueRunCondition(actorRunId: string, actorAgentId: string) {
  return sql<boolean>`exists (
    ...
    where ${heartbeatRuns.id} = ${actorRunId}
      ...
      and (${heartbeatRuns.contextSnapshot} ->> 'controllerEpoch')::bigint = (
...
export function issueService(db: Db) {
  ...
  return {
    ...
    listComments: async (
      ...
      const commentsWithResolvedNames = await resolveCommentAuthorNames(enrichedComments);
      return commentsWithResolvedNames.map((comment) => redactIssueComment(comment, censorUsernameInLogs));
    },
    ...
    getComment: async (commentId: string) => {
      ...
      const [commentWithResolvedName] = await resolveCommentAuthorNames([enrichedComment ?? comment]);
