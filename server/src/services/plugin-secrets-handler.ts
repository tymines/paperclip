...
export function createPluginSecretsHandler(
  ...
  return {
    async resolve(params: PluginSecretsResolveParams): Promise<string> {
      ...
      // ---------------------------------------------------------------
      // 2. Look up the secret to determine its owning company, then
      //    resolve via the Paperclip secret provider system.
      //    We self-host, so cross-company isolation is not a concern.
      //    Rate limiting (above) prevents brute-force UUID enumeration.
      // ---------------------------------------------------------------
      const secret = await secrets.getById(trimmedRef);
      if (!secret) {
        const err = new Error("Secret not found: " + trimmedRef);
