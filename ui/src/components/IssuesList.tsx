    ...
    <div className="border border-border bg-background p-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0 flex-1 space-y-2">
            ...
            {showCostSummary && (
              <>
                <span
                  ...
                  } across ${costSummary.issueCount} sub-${issueNoun.singular}${
          ...
          <div
            role="progressbar"
            aria-label={`Sub-${issueNoun.plural} completion progress`}
            aria-valuemin={0}
        ...
        <div className="min-w-0 border border-border bg-background px-3 py-2 text-sm lg:w-72">
          {target && targetIssue ? (
            ...
            <div className="text-sm font-medium text-foreground">No active sub-{issueNoun.plural}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">All sub-{issueNoun.plural} done</div>
          ) : (
            <div className="text-sm font-medium text-foreground">No actionable sub-{issueNoun.plural}</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  // Live agent runs for the Fleet Activity strip
  const { data: liveRuns } = useQuery({
    queryKey: [...queryKeys.liveRuns(selectedCompanyId ?? ""), "fleet-activity"],
  ...
  const runningAgentIds = useMemo(() => {
    ...
    for (const r of liveRuns ?? []) {
      if (r.status === "running" || r.status === "queued") ids.add(r.agentId);
    }
    return ids;
  }, [liveRuns]);
  ...
  const createActionLabel = createIssueLabel ? `Create ${createIssueLabel}` : `Create ${issueNoun.capSingular}`;
  const createButtonLabel = createIssueLabel ? `New ${createIssueLabel}` : `New ${issueNoun.capSingular}`;
  ...
  return (
    <div ref={rootRef} className="space-y-4">
      ...
      {/* Fleet Activity Strip — shows current agent status */}
      <FleetActivityStrip
        agents={agents}
...
/* -------------------------------------------------------------------------- */
/* Fleet Activity Strip — shows current agent status in the Tasks tab        */
/* -------------------------------------------------------------------------- */
function FleetActivityStrip({
  agents,
  runningAgentIds,
  selectedCompanyId,
}: {
  ...
  const active = useMemo(() => {
    if (!agents) return [];
    return agents
      .filter((a) => a.status !== "paused" && a.status !== "terminated" && a.status !== "archived")
      .sort((a, b) => {
        const aLive = runningAgentIds.has(a.id) ? 1 : 0;
        const bLive = runningAgentIds.has(b.id) ? 1 : 0;
        if (aLive !== bLive) return bLive - aLive;
        const aHb = a.lastHeartbeatAt ? new Date(a.lastHeartbeatAt).getTime() : 0;
        const bHb = b.lastHeartbeatAt ? new Date(b.lastHeartbeatAt).getTime() : 0;
        return bHb - aHb;
      });
  ...
  const getModel = (agent: Agent): string | null => {
    const m1 = (agent.adapterConfig as Record<string, unknown> | null)?.model;
    if (typeof m1 === "string" && m1.trim()) return m1.trim();
    const m2 = (agent.runtimeConfig as Record<string, unknown> | null)?.model;
    if (typeof m2 === "string" && m2.trim()) return m2.trim();
    return null;
  };

  const relativeTime = (iso: string | Date): string => {
  ...
  return (
    <div className="rounded-lg border px-4 py-3" style={{ borderColor: "hsl(var(--border))" }}>
      <div className="mb-2 flex items-center gap-2">
        <span className="text-xs font-semibold uppercase tracking-[0.08em]" style={{ color: "hsl(var(--muted-foreground))" }}>
          Fleet Activity
        </span>
        <span className="text-[10px] font-mono rounded px-1" style={{ background: "hsl(var(--muted))", color: "hsl(var(--muted-foreground))" }}>
          {active.length}
        </span>
      </div>
      <div className="flex flex-wrap gap-2">
        {active.map((agent) => {
          const live = runningAgentIds.has(agent.id);
          const model = getModel(agent);
          const hb = agent.lastHeartbeatAt ? relativeTime(agent.lastHeartbeatAt) : null;
          const statusColor =
            agent.status === "error"
              ? "hsl(var(--destructive))"
              : live
          ...
          return (
            <div
              ...
              style={{
                background: live ? "hsla(var(--success), 0.08)" : "hsl(var(--muted))",
                border: `1px solid ${live ? "hsla(var(--success), 0.2)" : "hsl(var(--border))"}`,
              }}
            >
