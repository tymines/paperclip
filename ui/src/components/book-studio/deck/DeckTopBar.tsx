...
export function DeckTopBar({ books, activeBookId, booksLoading = false, onSelectBook, onRenameBook, onNewBook, status, mode, onModeChange, onTaste, onRunPlan, onBrainstorm, onMedia, onExport }: {
  ...
  return (
    <header className="flex h-[52px] min-w-0 items-center gap-2 overflow-hidden border-b border-white/5 bg-[#0a0c10] px-2 @min-[1100px]/deck:gap-3.5 @min-[1100px]/deck:px-4">
      ...
      <select
        className="min-w-0 flex-1 truncate rounded-md border border-white/20 bg-[#171b24] px-2 py-1 text-xs text-white shadow-sm outline-none focus:border-[#e0955a] focus:ring-2 focus:ring-[#e0955a]/30 disabled:cursor-not-allowed disabled:bg-[#10131a] disabled:text-gray-600 @min-[1100px]/deck:max-w-[170px] @min-[1100px]/deck:flex-none"
        value={activeBookId ?? ""}
      ...
      <button className="hidden items-center rounded-md border border-white/15 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide hover:bg-white/5 @min-[1100px]/deck:inline-flex" onClick={onNewBook} title="Create a new book">+ New Book</button>
      <div className="flex-1" />
      {status && (
        <div className="hidden items-center gap-2 whitespace-nowrap text-[11.5px] text-gray-400 tabular-nums @min-[1100px]/deck:flex">
      ...
      <div className="hidden overflow-hidden rounded-md border border-white/15 @min-[1100px]/deck:flex" title="Director Mode autonomy — per-book">
        {MODES.map((m) => (
          <button
            key={m.id}
      ...
      <button className="hidden rounded-md border border-white/15 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide hover:bg-white/5 @min-[1100px]/deck:inline-flex" onClick={onTaste} title="Taste profile — standing generation rules (visible, editable)">Taste</button>
      <button className="hidden rounded-md bg-[#e0955a] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-[#181008] hover:bg-[#eaa96f] @min-[1100px]/deck:inline-flex" onClick={onRunPlan} title="Run Plan — approve before any autonomous drafting">Run Plan</button>
      <button className="grid h-11 w-11 shrink-0 place-items-center rounded-md border border-white/15 hover:bg-white/5 @min-[1100px]/deck:h-[30px] @min-[1100px]/deck:w-[30px]" onClick={onBrainstorm} title="Brainstorm chat" aria-label="Brainstorm"><Sparkles className="w-3.5 h-3.5" /></button>
      <button className="grid h-11 w-11 shrink-0 place-items-center rounded-md border border-white/15 hover:bg-white/5 @min-[1100px]/deck:h-[30px] @min-[1100px]/deck:w-[30px]" onClick={onMedia} title="Book media — cover, illustrations, trailer, narration, library" aria-label="Media"><Film className="w-3.5 h-3.5" /></button>
      <button className="hidden rounded-md border border-white/15 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide hover:bg-white/5 @min-[1100px]/deck:inline-flex" onClick={onExport} title="Export — Markdown · EPUB · PDF · Audiobook">Export</button>
      {renaming && activeBook && (
