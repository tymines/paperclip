...
export function DeckTopBar({ books, activeBookId, booksLoading = false, onSelectBook, onRenameBook, onNewBook, status, mode, onModeChange, onTaste, onRunPlan, onBrainstorm, onMedia, onExport }: {
  ...
  return (
    <header className="flex h-[52px] min-w-0 items-center gap-2 overflow-hidden border-b border-white/5 bg-[#0a0c10] px-2 @min-[1320px]/deck:gap-3.5 @min-[1320px]/deck:px-4">
      ...
      <button className="hidden shrink-0 items-center whitespace-nowrap rounded-md border border-white/15 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide hover:bg-white/5 @min-[1320px]/deck:inline-flex" onClick={onNewBook} title="Create a new book">+ New Book</button>
      <div className="flex-1" />
      {status && (
        <div className="hidden shrink-0 items-center gap-2 whitespace-nowrap text-[11.5px] text-gray-400 tabular-nums @min-[1320px]/deck:flex">
      ...
      <div className="hidden shrink-0 overflow-hidden rounded-md border border-white/15 @min-[1320px]/deck:flex" title="Director Mode autonomy — per-book">
        {MODES.map((m) => (
          <button
            key={m.id}
      ...
      <button className="hidden shrink-0 whitespace-nowrap rounded-md border border-white/15 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide hover:bg-white/5 @min-[1320px]/deck:inline-flex" onClick={onTaste} title="Taste profile — standing generation rules (visible, editable)">Taste</button>
      <button className="hidden shrink-0 whitespace-nowrap rounded-md bg-[#e0955a] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-[#181008] hover:bg-[#eaa96f] @min-[1320px]/deck:inline-flex" onClick={onRunPlan} title="Run Plan — approve before any autonomous drafting">Run Plan</button>
      ...
      <button className="hidden shrink-0 whitespace-nowrap rounded-md border border-white/15 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide hover:bg-white/5 @min-[1320px]/deck:inline-flex" onClick={onExport} title="Export — Markdown · EPUB · PDF · Audiobook">Export</button>
      {renaming && activeBook && (
