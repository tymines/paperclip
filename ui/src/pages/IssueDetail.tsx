...
export function IssueDetail() {
  ...
  const sourceBreadcrumb = useMemo(
    () => readIssueDetailBreadcrumb(issueId, location.state, location.search) ?? { label: issueNoun.capPlural, href: "/issues" },
    [issueId, location.state, location.search, issueNoun.capPlural],
  );
  ...
  const breadcrumbTitle = issue?.title ?? issueId ?? issueNoun.capSingular;
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          <DetailBackButton
            fallbackTo={backHref ?? (sourceBreadcrumb.label === "Tasks" ? "/tasks" : "/issues")}
            label={sourceBreadcrumb.label === "Tasks" ? "Back to board" : undefined}
          />
          <StatusIcon
            status={issue.status}
          ...
          {issue.workMode === "planning" ? (
            <span
              ...
              title={`This ${issueNoun.singular} is in planning mode.`}
            >
              Planning
            </span>
          ) : null}
            ...
            <Popover open={moreOpen} onOpenChange={setMoreOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  size="icon-xs"
                  className="shrink-0"
                  aria-label={`More ${issueNoun.singular} actions`}
                  title={`More ${issueNoun.singular} actions`}
                  onKeyDown={(event) => {
