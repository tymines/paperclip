...
export function ReelDetail() {
  ...
  const reelQuery = useQuery({
    queryKey: ["reels", selectedCompanyId, reelId],
    queryFn: () => reelsApi.get(selectedCompanyId!, reelId!),
    enabled: !!selectedCompanyId && !!reelId,
    refetchInterval: (query) => {
      const data = query.state.data;
      return data && isReelInProgress(data.reel) ? 4_000 : false;
    },
  });

  const regenMut = useMutation({
    mutationFn: (sceneIndex: number) =>
      reelsApi.regenerateScene(selectedCompanyId!, reelId!, sceneIndex),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["reels", selectedCompanyId, reelId] }),
  });
