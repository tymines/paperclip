...
export function Reels() {
  ...
  const reelsQuery = useQuery({
    queryKey: ["reels", selectedCompanyId],
    queryFn: () => reelsApi.list(selectedCompanyId!, { limit: 50 }),
    enabled: !!selectedCompanyId,
    // Auto-refetch while there are in-progress reels — polling for live status
    refetchInterval: (query) => {
      const reels = query.state.data?.reels ?? [];
  ...
  return (
    <div className="p-6">
      <header className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => queryClient.invalidateQueries({ queryKey: ["reels", selectedCompanyId] })}
      ...
      <NewReelDialog
        ...
        onCreated={() => queryClient.invalidateQueries({ queryKey: ["reels", selectedCompanyId] })}
      />
    </div>
  );
}
