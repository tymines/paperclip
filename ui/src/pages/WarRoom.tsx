// War Room — the phase-gated software-factory surface (WAR-ROOM-ROOMS-PIPELINE
// SPEC / WO-5). An 8-stage pipeline (Idea→Retro) with per-stage room chat, the
// artifact panel, and Tyler's manual gate; plus a "Hermes" view that embeds the
// chief-of-staff chat as the direct-line/council surface. Design System v1.0.
// (Supersedes the chat-only War Room — Tyler's phases ruling is canon.)
...
export function WarRoom() {
  ...
  const killM = useMutation({
    mutationFn: (args: { runId: string; reason: string }) => pipelineApi.kill(cid!, args.runId, args.reason),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["pipeline-runs", cid] });
      qc.invalidateQueries({ queryKey: ["pipeline-run", cid, selectedRunId] });
      setKillOpen(false); setKillReason("");
      pushToast({ title: "Run killed", variant: "success" } as any);
    },
  });
        ...
        {([["pipeline", "Pipeline", GitBranch], ["hermes", "Hermes", MessageSquare]] as const).map(([v, label, Icon]) => (
          <button key={v} onClick={() => setView(v)}
            style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 999, cursor: "pointer", fontSize: 12.5,
              ...
              border: `1px solid ${view === v ? DS.primary : DS.border}` }}>
            <Icon size={13} /> {label}
          </button>
        ))}
      </div>
          ...
          {!selectedRunId || !run ? (
            ...
            <>
              <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 18, flexWrap: "wrap" }}>
                ...
                {(() => {
                  const bounced = stages.length - new Set(stages.map((x: any) => x.name)).size; // extra run_stages rows = re-entries
                  return bounced > 0 ? (
                    <span title="Times a stage has bounced back for rework"
                      style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontFamily: DS_MONO, color: DS.amber, background: `${DS.amber}18`, border: `1px solid ${DS.amber}44`, borderRadius: 999, padding: "3px 9px" }}>
                      ↺ {bounced} rework{bounced === 1 ? "" : "s"}
                    </span>
                  ) : null;
                })()}
                {run.status !== "completed" && run.status !== "killed" && (
                  <button onClick={() => setKillOpen(true)} title="Kill this run"
                    style={{ marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 5, background: "transparent", border: `1px solid ${DS.critical}55`, color: DS.critical, borderRadius: 8, padding: "5px 10px", fontSize: 12, cursor: "pointer" }}>
                    <Skull size={13} /> Kill run
                  </button>
                )}
              </div>
                ...
                <div style={{ position: "relative", padding: "6px 8px 20px", minWidth: 520 }}>
                  ...
                  <div style={{ position: "relative", display: "flex", justifyContent: "space-between" }}>
                    {STAGES.map((s) => {
                      ...
                      const iters = stages.filter((x: any) => x.name === s).length; // each rework re-enters a row
                      const c = stageColor(st?.status);
                      const capHit = iters >= 3; // revision-loop cap: 3 iterations before escalation
                      return (
                        <div key={s} style={{ flex: 1, textAlign: "center" }}>
                          <div style={{
                            ...
                            {STAGES.indexOf(s) + 1}
                            {iters > 1 && (
                              <span title={`${iters - 1} rework${iters - 1 === 1 ? "" : "s"}${capHit ? " — cap reached, escalate" : ""}`}
                                style={{ position: "absolute", top: -7, right: -9, fontSize: 9, fontFamily: DS_MONO, fontWeight: 700,
                                  color: "#fff", background: capHit ? DS.critical : DS.amber, borderRadius: 999, padding: "0 4px", lineHeight: "13px", border: `1px solid ${DS.canvas}` }}>
                                ↺{iters - 1}
                              </span>
                            )}
                          </div>
                ...
                <div style={{ background: `${DS.primary}0E`, border: `1px solid ${DS.primary}44`, borderRadius: 12, padding: 16, maxWidth: 560 }}>
                    ...
                    {currentIdx > 0 && (
                      <>
                        <Button variant="destructive" disabled={!reason.trim()}
                          title={!reason.trim() ? "A reason is required to send a stage back" : "Send back"}
                  ...
                  {currentIdx > 0 && (
                    <>
                      ...
                      {!reason.trim() && <div style={{ fontSize: 10.5, color: DS.textFaint, marginTop: 4 }}>A gate rejection must carry a reason — it's recorded on the run.</div>}
                    </>
                  )}
                </div>
              )}
      ...
      <Dialog open={killOpen} onOpenChange={setKillOpen}>
        <DialogContent>
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="Why are you killing it?" value={killReason} onChange={(e) => setKillReason(e.target.value)} />
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setKillOpen(false)}>Cancel</Button>
            <Button variant="destructive" disabled={!selectedRunId || killM.isPending}
              onClick={() => selectedRunId && killM.mutate({ runId: selectedRunId, reason: killReason })}>
              Kill run
            </Button>
          </DialogFooter>
        </DialogContent>
