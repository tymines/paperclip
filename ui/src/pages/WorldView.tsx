...
interface FireItem { lat: number; lon: number; brightness: number | null; confidence: string; frp: number | null; satellite: string; instrument: string }
...
async function getFirms(): Promise<FireItem[]> {
  try {
    const r = await fetch(`${COLLECTOR}/firms`, { signal: AbortSignal.timeout(10000) });
    if (!r.ok) return [];
...
function WorldMap({ quakes, fires, layers }: {
  quakes: Quake[];
  fires: FireItem[];
  layers: { seismic: boolean; major: boolean; grid: boolean; borders: boolean; fires: boolean };
}) {
  ...
  return (
    <svg viewBox={`0 0 ${WORLD_W} ${WORLD_H}`} preserveAspectRatio="xMidYMid slice"
      ...
      {layers.fires && fires.map((f, i) => {
        ...
        const intensity = f.frp ? Math.min(f.frp * 0.06, 5) : 2;
        return (
          <g key={`fire-${i}-${f.lat}-${f.lon}`}>
            <circle cx={x} cy={y} r={intensity + 1.5} fill={C.amber} opacity={0.25} />
            <circle cx={x} cy={y} r={Math.max(1, intensity)} fill={C.red} opacity={0.85}>
              <title>{`${f.satellite}/${f.instrument} · FRP: ${f.frp?.toFixed(1) || "?"} MW · ${f.confidence}`}</title>
            </circle>
          </g>
        );
      })}
...
export function WorldView() {
  ...
  const [layers, setLayers] = useState({ seismic: true, major: true, fires: true, grid: true, borders: true });
  ...
  const firms = useQuery({ queryKey: ["worldview", "firms"], queryFn: getFirms, refetchInterval: 300000, retry: 0, staleTime: 120000 });
      ...
      <section style={{ borderBottom: `1px solid ${C.line}`, background: C.panel }}>
        <header className="flex items-center gap-2 px-2.5 py-1.5" style={{ borderBottom: `1px solid ${C.line}` }}>
          ...
          <span className="ml-auto flex items-center gap-3 text-[10px] uppercase tracking-wider" style={{ color: C.faint }}>
            ...
            {fr.length > 0 && <span className="flex items-center gap-1.5"><Dot color={C.amber} />{fr.length} FIRES</span>}
          </span>
        </header>
        ...
        <div className="relative w-full overflow-hidden" style={{ height: "min(54vh, 560px)", minHeight: 320, background: C.ocean }}>
          {quakes.isError
            ...
            : <WorldMap quakes={qk} fires={fr} layers={layers} />}
